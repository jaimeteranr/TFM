...
def generar_calendario(
    ...
    # =====================================
    # LIMITAR TRABAJADORES
    # =====================================

    if LIMITAR_TRABAJADORES:

        if numero_trabajadores is None:
        ...
        if numero_trabajadores <= 0:
            ...
            raise ValueError(
                "El número de trabajadores debe ser mayor que 0."
            )

        if numero_trabajadores > len(activos):

            raise ValueError(
                f"Se solicitan {numero_trabajadores} "
                f"trabajadores, pero solo hay "
                f"{len(activos)} activos."
            )

        activos = activos.head(
    ...
    if not USAR_TURNOS_BLOQUEADOS:
        ...
        turnos_bloqueados = (
            turnos_bloqueados.iloc[0:0].copy()
        )
    ...
    if turnos_bloqueados is not None:
