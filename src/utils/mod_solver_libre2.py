...
class SolverLibre:
    ...
    def resolver(
        ...
        patrones = turnos_libres.copy()

        patrones = patrones.rename(
            columns={
                "turno_id": "patron_id",
        ...
        cobertura_patrones = cobertura_turnos.rename(
            columns={
                "turno_id": "patron_id"
            }
        ).copy()
        ...
        #     if cierre <= pd.to_datetime(
        #         horarios_base[dia]["apertura"],
        #         format="%H:%M"
        #     ):
        #         cierre += pd.Timedelta(days=1)

        #     cierre += pd.Timedelta(
        #         minutes=reglas["minutos_recogida"]
        #     )
        ...
        for dia in dias:
            ...
            hora_cierre = horarios_base[dia]["cierre"]
            ...
            for w in activos["id"]:
                ...
                for p in patrones["patron_id"]:
                    ...
                    if hora_cierre_real in patron_horas[p]:
        ...
        for i, (_, fila) in enumerate(demanda.iterrows()):
